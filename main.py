...
# Rules (硬性约束)
1. 【禁止出现“早报”、“7点”等字眼】：因为这是用户手动随时点火运行的。请统一使用给定的【当前手动点火时间段】作为时间缀。
2. 【严格遵循一整套排版格式】：输出的内容必须完全匹配以下制式。请特别注意“体感温度”的子集层级关系：
   ...
   # 今日{{当前手动点火时间段}}天气决策简报
   ## 1. 未来1小时出门安排
   - 实际体感温度：(在此处根据当前温湿度和风速，计算出皮肤真正感受到的温度，并用一两个词形容如“偏黏糊/干爽/闷热”)
   (接下来另起一行输出大白话内容：结合体感进行【未来一小时短时状况】的具体描述，说明是否有突发降水狂风、给出骑行或步行建议)
   
   ## 2. 全天穿搭与防雨防晒规划
   (大白话内容：必须包含明确具体的【穿衣建议】；针对全天温差和紫外线的生活指南；具体的【随身物品建议】)
   
   ## 3. 今日大自然追光预测
   (大白话内容：结合云量和能见度自主推演今天是否有几率触发特殊自然现象。无奇观时可以写“由于云量过厚，今日各大奇观概率均低于20%...”；有奇观且几率大于70%时，直接升级为【高能奇观预警】并描述视觉画面)

3. 【字数与语气】：用充满生活温度、专业且接地气的大白话撰写。除格式要求的标志外，拒绝任何多余的废话前缀。
...
[DATA_END]"""
    ...
    try:
        ...
        msg["Subject"] = f"⏰ 您的{time_tag}天气决策简报" # 邮件主题也改为了简报
        msg["From"] = email
        msg["To"] = email
